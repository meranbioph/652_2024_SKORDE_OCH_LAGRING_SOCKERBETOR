...
#
# Version 2024-10-21
# This is the 2024 version of the model. Updates include:
# - 2024 price model.
# - weather data imported for current year from Nifti, with the rest of the filled with the average 2016-23
...
sek_eur <- 11.3576 # source: "Eurokursen 2023", på sockerbetor.nu 1 oktober 2024
kr_tonne <- 39.47*sek_eur # 1-year fixed price, at reference pol of 17%
kr_renhet <- 2 # extra kr per %-point over ref (93.15%), at 17% pol%. Source: Branchavtal 2024/25
...
kr_vol <- 3.00 # € extra per tonne, as fraction of previous year. Branchavtal 17.1
...
date_full <- seq(as.POSIXct("2024-09-10", tz = time_zone, format = "%Y-%m-%d"), length.out = 172, by = "1 day") # Ends 28 Feb.
price_early <- c(37,37,37,37,37,36,33,30,27,24,21,19,16,14,12,10,8,6,4,2,1,rep(0,151)) #Source: Branchavtal 2024/25 - unchanged 2024
price_late <- c(rep(0,82),seq(1,23,by=1),seq(24.5,124,by=1.5)) #Source: Branchavtal 2024/25. Unchanged 2024
...
ref_pol <- 0.17 #Source: Branchavtal 2024/25 - unchanged
ref_renhet <- 0.9315 #Source: Branchavtal 2024/25 - unchanged
ref_TT_1 <- as.POSIXct("2024-11-15", tz = time_zone, format = "%Y-%m-%d") # 5kr/tn beets
ref_TT_2 <- as.POSIXct("2024-11-23", tz = time_zone, format = "%Y-%m-%d") # 10kr/tn beets
ref_TT_3 <- as.POSIXct("2024-12-01", tz = time_zone, format = "%Y-%m-%d") # 15kr/tn beets
ref_early <- as.POSIXct("2024-09-30", tz = time_zone, format = "%Y-%m-%d") # last day the early payment is made for
ref_late_1 <- as.POSIXct("2024-12-01", tz = time_zone, format = "%Y-%m-%d") #first day you get money for late delivery
ref_late_2 <- as.POSIXct("2024-12-24", tz = time_zone, format = "%Y-%m-%d") #first day you get 1.5 x money for late delivery
...
# Create temperature series to test the model on. Data taken from NBRs 5 stations, through to 28 Feb.
temp_tab_historical <- data.frame(
  ...
  yr2023 = c(18.86,16.62,18,15.88,13.74,14.48,16.32,16.7,18.64,16.72,17.42,18.68,16.72,14.24,13.16,13.9,14.72,16,16.86,16.16,14.4,14.12,16.16,15.62,13.56,12.32,13.68,11.42,6.78,9.14,11.86,14.7,11.06,11.7,11.44,8.64,7.36,7.94,9.32,5.8,5.2,7.44,10.04,9.68,8.38,8.4,6.76,5.44,5.52,9.08,10.46,8.28,5.46,9.98,9.54,8.34,8.6,9.4,9.24,8.84,8.64,7.44,7.12,6.18,4.34,5.74,4.1,4.84,3.44,0.54,3.7,4.6,2.16,1.04,7.78,1.94,-1.64,-3.78,-4.54,-6.62,-4.36,-2.5,-5.24,-5.44,-5.22,-3.92,0.46,1.1,0.26,1.12,1.7,2.86,2.56,0.6,-0.32,-0.72,3.24,6.36,7.78,7.46,6.44,3.66,4.46,1.82,0.26,2.8,5.74,4.42,2.18,6.14,6.72,4.76,3.96,4.24,1.44,-0.28,-4.34,-10.46,-8.4,-7.6,-7.38,-3.42,-1.52,-1.48,-2.02,1.06,1.24,-3.5,-8.84,-0.72,-2.84,-0.14,1.66,1.82,4.58,4.64,6.1,4.02,3.32,4.74,3.2,3.26,3.5,4.4,4.46,4.9,7.02,5.46,3.8,2.98,0.52,0.7,-0.22,0.78,1.02,1.36,3.34,4.14,4.74,7.92,4.2,1.94,3.94,5.64,5.68,5.52,5.9,4.52,4.3,3.44,0.14,2.18)
)
